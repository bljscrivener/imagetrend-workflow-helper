  ...
  function inspect(choice){
    ...
    if(repeated){
      ...
      if(collectionPath.endsWith('PatientProcedures')){
        const code=ko.unwrap(resolve(entry,'PatientProcedureProcedurePerformedModValue.ProcedurePerformed',ko));
        const names=(app.formComposer.agencyResources?.['02dffd5f-4c68-506b-881d-5b00c78090aa']?.Elements||[]).filter(x=>x.Id===code);
        if(names.length!==1||!['Assessment -ALS','Neurological assessment','Adult pain assessment','Moving a patient to a stretcher'].includes(norm(names[0].Value)))throw Error('Existing procedure is outside the A15 bundle.');
      }
    ...
    const matches=(resources?.[special?'NotValues':'Elements']||[]).filter(e=>norm(e.Value)===norm(c.target));
