...
// @version      0.3.3
// @description  Universal read-only DOM investigator with copy-first iPad handoff, normalized captures, help, and ImageTrend metadata adapter.
// @match        *://*/*
// @grant        none
// @run-at       document-idle
// @noframes
// @updateURL    https://raw.githubusercontent.com/bljscrivener/imagetrend-workflow-helper/gfi-0.3.3/src/gremlin-universal-investigator.user.js
// @downloadURL  https://raw.githubusercontent.com/bljscrivener/imagetrend-workflow-helper/gfi-0.3.3/src/gremlin-universal-investigator.user.js
// ==/UserScript==
...
(() => {
    ...
    return {
      ...
      data:{
        ...
        ancestors:(() => {
          const arr = [];
          let node = el;
          for (let depth = 0; node && node.nodeType === 1 && depth < 8; depth += 1, node = node.parentElement) arr.push(elementRecord(node, eventType));
          return arr;
        })()
      },
      notes:[
        ...
        'GFI does not invoke site actions, setters, clicks, submits, or field writes.'
      ]
    };
  }
  ...
  function capturePage() {
    ...
    return {
      ...
      notes:['Read-only page scan.', 'Input values are not exported.', 'Clipboard is the default iPad handoff; file download is backup only.']
    };
  }
  ...
  async function copyReport(report) {
    return copyText(reportText(report));
  }
  ...
  root.innerHTML = `
    <style>
      ...
      .panel{width:244px;background:#111d29;color:#eef6ff;border:1px solid #52708c;border-radius:12px;box-shadow:0 8px 26px #0006;overflow:hidden}
    ...
    <div class="panel">
      ...
      <div class="body">
        ...
        <button id="copy" class="primary">Copy for ChatGPT</button><button id="download">Download backup</button>
        <button id="clear" class="danger">Clear last</button><button id="help">Help</button>
        <div id="msg" class="msg">Ready. Clipboard is the primary iPad handoff.</div>
      </div>
      <div id="helpbox" class="help"><b>Inspect element</b>: arms one passive read-only pointer/touch capture.<br><br><b>Scan page</b>: inventories the current DOM and bindings without changing the page.<br><br><b>Copy for ChatGPT</b>: copies the complete newest normalized capture to the clipboard. Return to the same ChatGPT thread and paste it there. This is the default iPad workflow.<br><br><b>Download backup</b>: saves the same capture as TXT only when clipboard transfer is impractical.<br><br><b>Clear last</b>: drops only GFI's in-memory capture reference.<br><br><b>Policy</b>: GFI is observational. It never writes chart data or invokes ImageTrend actions.</div>
    </div>`;
  ...
  function handleInspectEvent(ev) {
    ...
    disarm();
    msg(`Captured ${lastReport.data.element.selector || lastReport.data.element.tag}. Tap Copy for ChatGPT.`);
  }
  ...
  $('pick').addEventListener('click', () => {
    ...
    msg(inspectArmed ? 'Inspection armed. Tap a target normally.' : 'Inspection cancelled.');
  });

  $('page').addEventListener('click', () => {
    lastReport = capturePage();
    msg(`Page scan captured ${lastReport.data.counts.candidates} candidates. Tap Copy for ChatGPT.`);
  });

  $('copy').addEventListener('click', async () => {
    if (!lastReport) return msg('No capture available.');
    const ok = await copyReport(lastReport);
    msg(ok ? 'Copied. Return to this ChatGPT thread and paste.' : 'Clipboard copy failed. Use Download backup.');
  });

  $('download').addEventListener('click', () => {
    if (!lastReport) return msg('No capture available.');
    downloadReport(lastReport);
    msg(`Backup saved: ${lastReport.header.exportHints.suggestedFilename}`);
  });
  ...
  window.GFI = Object.freeze({
    ...
    copyLast:() => lastReport ? copyReport(lastReport) : Promise.resolve(false),
