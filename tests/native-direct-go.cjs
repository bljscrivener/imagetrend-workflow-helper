...
(async()=>{
 ...
 await h.locator('#apply').click();await p.waitForFunction(()=>!document.querySelector('#it-a15-native-test').shadowRoot.querySelector('#apply').disabled);assert.deepEqual(await p.evaluate(()=>entries.vitals.map(x=>x.VitalsLevelOfResponseModValue.LevelOfResponse)),['alert','alert']);assert.equal(await p.evaluate(()=>writes.length),8);assert.equal(await h.locator('#tally').evaluate(x=>x.open),true);await h.locator('#hide').click();assert.equal(await h.locator('section').isVisible(),false);await h.locator('#launch').click();assert.equal(await h.locator('footer #clear-entries').count(),1);assert.equal(await h.locator('#apply').isEnabled(),true);
 ...
 await h.locator('#apply').click();await p.waitForFunction(()=>!document.querySelector('#it-a15-native-test').shadowRoot.querySelector('#apply').disabled);assert.equal(await p.evaluate(()=>writes.length),8,'rerun is idempotent');
 await p.evaluate(()=>{entries.vitals[0].VitalsLevelOfResponseModValue.LevelOfResponse='verbal';Storage.prototype.setItem=()=>{throw Error('storage unavailable');};});await h.locator('#apply').click();await p.waitForFunction(()=>document.querySelector('#it-a15-native-test').shadowRoot.querySelector('#result').textContent.includes('storage unavailable'));assert.equal(await p.evaluate(()=>writes.length),8,'capture failure prevents writes');
