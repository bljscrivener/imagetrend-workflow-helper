...
(async()=>{
 ...
 await h.locator('#launch').click();assert.equal(await h.locator('#ack').count(),0);assert.equal(await h.locator('#apply').isEnabled(),true);assert.equal(await h.locator('#tally').getAttribute('open'),null);assert.equal(await h.locator('#hide').textContent(),'Expand');
 await h.locator('#apply').click();assert.deepEqual(await p.evaluate(()=>entries.vitals.map(x=>x.VitalsLevelOfResponseModValue.LevelOfResponse)),['alert','alert']);assert.equal(await p.evaluate(()=>writes.length),8);assert.equal(await h.locator('#tally').evaluate(x=>x.open),true);await h.locator('#hide').click();assert.equal(await h.locator('#tally').evaluate(x=>x.open),false);assert.equal(await h.locator('#top-controls #clear-entries').count(),1);assert.equal(await h.locator('#apply').isEnabled(),true);
