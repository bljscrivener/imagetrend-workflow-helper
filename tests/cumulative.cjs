...
(async()=>{
 ...
 const results=await page.evaluate(async code=>{
  ...
  document.body.innerHTML='<div id="form-composer"><div class="single-row-control" id="late"><div class="koSingleselect-selectedItem-value"></div><div class="koSingleselect-selectedItem" data-bind="click: toggleDropDown">Open</div></div></div>';
  const late=document.getElementById('late');
  late.querySelector('.koSingleselect-selectedItem').onclick=()=>setTimeout(()=>{const option=document.createElement('div');option.className='koSingleselect-dropDownItem';option.textContent='Exact target';option.onclick=()=>late.querySelector('.koSingleselect-selectedItem-value').textContent='Exact target';late.append(option);},400);
  await api.setChoice(late,'Exact target','');
  check('delayed single-select options via selected-item opener',api.readField(late)==='Exact target');
  return results;
 },code);
