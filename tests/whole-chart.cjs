...
(async()=>{
 ...
 await page.evaluate(()=>{
  ...
  function render(name){
   ...
   if(rules[name]){
    ...
    b.onclick=()=>{window.model[name]=value;b.classList.add('selected');};
   }
 ...
 assert.equal(await page.locator('#panel-header').textContent(),'STAT Info');
 console.log('PASS whole-chart read-only review, unchecked bundle, cross-section apply and return');
 await browser.close();
