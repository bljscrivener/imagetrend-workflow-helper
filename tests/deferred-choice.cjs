...
(async()=>{
 ...
 await page.evaluate(()=>{
  ...
  function render(name){
   ...
   if(rules[name]){
    ...
    b.onclick=()=>{window.model[name]=value;b.classList.add('selected');}; if(name==='STAT Info'&&!window.model['Transport Info'])b.style.display='none';
   }
 ...
 assert.equal(await page.locator('#panel-header').textContent(),'STAT Info');
 console.log('PASS missing prerequisite deferred, independent sections applied, deferred choice retried');
 await browser.close();
