...
(async()=>{
 ...
 assert.equal(await p.evaluate(()=>new Date(entries.procedures[0].PatientProcedureProcedureDateTimeModValue.ProcedureDateTime).getHours()),11);await p.evaluate(()=>window.confirm=()=>false);await h.locator('#clear-entries').click();assert.equal(await p.evaluate(()=>entries.procedures.length),2);await p.evaluate(()=>{window.confirmCount=0;window.confirm=()=>{confirmCount++;return true;};});await h.locator('#clear-entries').click();assert.equal(await p.evaluate(()=>confirmCount),2);assert.equal(await p.evaluate(()=>entries.procedures.length),0);assert.equal(await p.evaluate(()=>entries.vitals.length),0);assert.equal(await p.evaluate(()=>model.Incident.DispatchSentinel),'keep');assert.equal(await p.locator('input[id="29336Time"]').inputValue(),'11:00:00');console.log('PASS timestamp rule, clear cancellation, double confirmation, dispatch/timeline preserved; all existing vital entries, reviewed replacements, reorder identity, unrelated procedure preserved, numbers protected, no duplicates, removed entry blocked');await b.close();
