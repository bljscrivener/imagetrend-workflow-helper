...
(async()=>{
 ...
 await p.route('https://pafford.imagetrendelite.com/**',r=>r.fulfill({body:'<div id="form-composer">Unrelated section</div>'}));await p.goto('https://pafford.imagetrendelite.com/');
 await p.evaluate(()=>{
  ...
  vitals.forEach(x=>x.GCSQualifiers=[]);window.entries={vitals,procedures};window.writes=[];vitals.forEach(x=>x.__parent=root);
  const resources={
   ...
   '02dffd5f-4c68-506b-881d-5b00c78090aa':{Elements:[{Id:'als',Value:'Assessment -ALS'},{Id:'other',Value:'Other procedure'}]}
  };
 ...
 assert.equal(await p.evaluate(()=>writes.length),7);assert.deepEqual(await p.evaluate(()=>entries.vitals.map(x=>x.GCSQualifiers.length)),[1,1]);
 ...
 await p.evaluate(()=>entries.vitals.shift());await h.locator('#apply').click();assert.equal(await p.evaluate(()=>writes.length),7);assert.deepEqual(await p.evaluate(()=>entries.vitals.map(x=>x.GCSQualifiers.length)),[1]);assert.match(await h.locator('#result').textContent(),/removed or ambiguous/);
 console.log('PASS all existing vital entries, reviewed replacements, reorder identity, unrelated procedure preserved, numbers protected, no duplicates, removed entry blocked');await b.close();
